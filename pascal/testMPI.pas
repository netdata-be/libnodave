...
procedure usage;
begin
    writeln('Usage: testMPI [-d] [-w] serial port.');
    ...
    writeln('-z will read some SZL list items (diagnostic information).');
    writeln('-2 uses a slightly different version of the MPI protocol. Try it, if your Adapter doesnt work.');
    writeln('-3 uses a different version of the MPI protocol. Try it, if your Adapter doesnt work. This is very experimental!');
    writeln('-4 uses a another different version of the MPI protocol. Try if your Adapter doesnt work.');
    writeln('-n will test newly added functions.');
    writeln('-a will read out everything from system state lists(SZLs).');
    ...
    writeln('--mpi=<number> will use number as the MPI adddres of the PLC. Default is 2.');
    writeln('--local=<number> will set the local MPI adddres to number. Default is 0.');

{$ifdef UNIX_STYLE}    
    writeln('Example: testMPI -w /dev/ttyS0');
{$endif}    
{$ifdef WIN32}
    writeln('Example: testMPI -w COM1');
{$endif}
end;

procedure readSZL(dc:pdaveConnection;id:longint; index:longint);
var res, SZLid, indx, SZcount, SZlen,i,j,len,dd: longint;
    d:array[0..3000] of byte;
begin
    writeln('Trying to read SZL-ID %04X index %02X.',id,index);
    res:=daveReadSZL(dc,id,index, @d, 3000);
    writeln('Function result: ',res,' ',daveStrerror(res),' len: ',dc^.AnswLen);
    _daveDump('Data',@d,dc^.AnswLen);
    
    if dc^.AnswLen>=4 then begin
	dd:=8;
	len:=dc^.AnswLen-8;
	SZLid:=$100*d[0]+d[1];
        indx:=$100*d[2]+d[3];
	writeln('result SZL ID ',SZLid,' ',indx);
	if dc^.AnswLen>=4 then begin
    	    SZlen:=$100*d[4]+d[5];
    	    SZcount:=$100*d[6]+d[7];
	    writeln(SZCount,' elements of ',SZlen,' bytes');
	    if len>0 then begin
		for i:=0 to SZcount-1 do begin
		    if len>0 then begin
			for j:=0 to SZlen-1 do begin
			    if len>0 then begin
				write(hexstr(d[dd],2),' ');
				dd:=dd+1;
			    end;
...
procedure readSZLAll(dc: pdaveConnection);
var
    d:array[0..1000] of byte;
    res, SZLid, indx, SZcount, SZlen,i,j, rid, rind,dd:longint;
begin    
    res:=daveReadSZL(dc,0,0,@d, 1000);
    writeln(res,' ',dc^.AnswLen);
    if dc^.AnswLen>=4 then begin
{	d=dc->resultPointer;
        dd=SzlList;
	dd+=8;}
	...
	if dc^.AnswLen>=8 then begin
		...
		writeln('\nID:',hexstr(rid,4),' Index:',hexstr(rind,2));
		readSZL(dc, rid, rind);
		for j:=0 to SZlen-1 do begin
		    write(hexstr(d[dd],2),' ');
		    dd:=dd+1;
	...
	doRun,doStop,
	doSZLread,doSZLreadAll,
	res, saveDebug,plcMPI, localMPI, speed, useProtocol: longint;
    d: single;
    di:pdaveInterface;
    dc:pdaveConnection;
    rs:daveResultSet;
...
USEC:=0;
	...
	if paramstr(adrPos)='-d' then begin
	    ...
	    writeln('setting PLC MPI adress to: ',plcMPI);
	end 
	...
	else if paramstr(adrPos)='-z' then begin
	    doSZLread:=1;
	end 
	else if paramstr(adrPos)='-a' then begin
	    doSZLreadAll:=1;
	end 
	...
	else if paramstr(adrPos)='-3' then begin
	    useProtocol:=daveProtoMPI3;
	end 
	else if paramstr(adrPos)='-4' then begin
    ...
    fds.rfd:=setPort(COMname,'38400','O');
    fds.wfd:=fds.rfd;
