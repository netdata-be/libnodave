...
int DECL2 stdwrite(daveInterface * di, uc * buffer, int length) {
    ...
    if (daveDebug & daveDebugByte)
	_daveDump("I send",buffer,length);
//	PurgeComm(di->fd.rfd, PURGE_RXCLEAR);  //******* this is what microwin does
    EscapeCommFunction(di->fd.rfd, CLRRTS);  //******* this is what microwin does (needed for usb-serial)
//	PurgeComm(di->fd.rfd, PURGE_RXCLEAR);  //******* this is what microwin does
//	PurgeComm(di->fd.rfd, PURGE_TXCLEAR);  //******* this is what microwin does
    WriteFile(di->fd.rfd, buffer, length, &i,NULL);
//    EscapeCommFunction(di->fd.rfd, CLRRTS);  //******* this is what microwin does
    return i;
}
#endif 
